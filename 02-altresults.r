...
gg_dropone <- ggplot(dropone_dt,aes(x=country,y=est))+
  ...
  facet_grid(.~event)+
  coord_flip()+
  labs(title="",x="Omitted country",y="Harvest-time change in conflict incidents relative to the baseline (%)")+
  theme_paper()+
...
ggsave("Figures/dropacountry_incidents.png",gg_comb,width=6.5,height=4.5,dpi="retina",device="png")

ggsave("Figures/dropacountry_incidents.eps",gg_comb,width=6.5,height=4.5,dpi="retina",device="eps")
...
gg_dropone <- ggplot(dropone_dt,aes(x=year,y=est))+
  ...
  facet_grid(.~event)+
  coord_flip()+
  labs(title="",x="Omitted year",y="Harvest-time change in conflict incidents relative to the baseline (%)")+
  theme_paper()+
  theme(panel.grid.major.y=element_blank(),panel.grid.major.x=element_line(colour="darkgray"),axis.text.y=element_text(hjust=0))
...
ggsave("Figures/dropayear_incidents.png",gg_comb,width=6.5,height=4.5,dpi="retina",device="png")

ggsave("Figures/dropayear_incidents.eps",gg_comb,width=6.5,height=4.5,dpi="retina",device="eps")
...
gg_shuffle <- ggplot(shuffle_dt,aes(x=iter,y=est))+
  ...
  labs(title="",x="Iteration",y="Harvest-time change in conflict incidents relative to the baseline (%)")+
  theme_paper()+
...
ggsave("Figures/shuffleharvest_incidents.png",gg_comb,width=6.5,height=7.0,dpi="retina",device="png")

ggsave("Figures/shuffleharvest_incidents.eps",gg_comb,width=6.5,height=7.0,dpi="retina",device="eps")


# 02 - Monthly ----

impact1m <- function(x){
  r <- feols(incidents~area:(l2+l1+s0+f1+f2)+rain_t | xy+country^year+yearmo, data=x,vcov=~xy)
  ...
  return(list(descriptive=c(incidents=round(m$incidents,2),cropland=round(m$cropland,2)),effect=c(h_est[1],h_std[1],h_est[2],h_std[2],h_est[3],h_std[3],h_est[4],h_std[4],h_est[5],h_std[5]),output=c(h_coef[1],h_se[1],h_coef[2],h_se[2],h_coef[3],h_se[3],h_coef[4],h_se[4],h_coef[5],h_se[5])))
}
...
## effect
coef0_fe <- feols(incidents~area:(l2+l1+s0+f1+f2)+rain_t | xy+country^year+yearmo, datasub_dt,vcov=~xy)
...
## effect
coef1_fe <- feols(incidents~area:(l2+l1+s0+f1+f2)+rain_t | xy+country^year+yearmo, datasub_dt[event=="battles"],vcov=~xy)
coef2_fe <- feols(incidents~area:(l2+l1+s0+f1+f2)+rain_t | xy+country^year+yearmo, datasub_dt[event=="violence"],vcov=~xy)
coef3_fe <- feols(incidents~area:(l2+l1+s0+f1+f2)+rain_t | xy+country^year+yearmo, datasub_dt[event=="riots" ],vcov=~xy)
coef4_fe <- feols(incidents~area:(l2+l1+s0+f1+f2)+rain_t | xy+country^year+yearmo, datasub_dt[event=="protests"],vcov=~xy)
...
gg_monthly <- ggplot(long_dt,aes(x=period,y=est,group=event))+
  ...
  labs(title="",x="Month from harvest",y="Harvest-time change in conflict incidents relative to the baseline (%)")+
  theme_paper()
...
impact1d <- function(x){
  ...
  m <- x[,.(incidents=mean(incidents)),by=.(area)]
  m <- m[order(area)]
  ...
  return(list(descriptive=c(incidents=round(m$incidents,2)),effect=c(h_est[1],h_std[1],h_est[2],h_std[2],h_est[3],h_std[3]),output=c(h_coef[1],h_se[1],h_coef[2],h_se[2],h_coef[3],h_se[3])))
}
...
impact3 <- function(x){
  r <- feols(incidents~area:seas:irri+rain_t | xy+country^year+yearmo, data=x,vcov=~xy)
  ...
  return(list(descriptive=c(incidents=round(m$incidents,2),cropland=round(m$cropland,2)),effect=c(h_est[1],h_std[1],h_est[2],h_std[2]),output=c(h_coef[1],h_se[1],h_coef[2],h_se[2])))
}
...
## effect
coef0_fe <- feols(incidents~area:seas:irri+rain_t | xy+country^year+yearmo, datasub_dt,vcov=~xy)
...
## effect
coef1_fe <- feols(incidents~area:seas:irri+rain_t | xy+country^year+yearmo, datasub_dt[event=="battles"],vcov=~xy)
coef2_fe <- feols(incidents~area:seas:irri+rain_t | xy+country^year+yearmo, datasub_dt[event=="violence"],vcov=~xy)
coef3_fe <- feols(incidents~area:seas:irri+rain_t | xy+country^year+yearmo, datasub_dt[event=="riots" ],vcov=~xy)
coef4_fe <- feols(incidents~area:seas:irri+rain_t | xy+country^year+yearmo, datasub_dt[event=="protests"],vcov=~xy)
...
impact4 <- function(x){
  r <- feols(incidents~area:seas:city_cat+rain_t | xy+country^year+yearmo, data=x,vcov=~xy)
  
  m <- x[area>0,.(incidents=mean(incidents),cropland=mean(area)),by=.(city_cat)]
...
## effect
coef0_fe <- feols(incidents~area:seas:city_cat+rain_t | xy+country^year+yearmo, datasub_dt,vcov=~xy)
...
## effect
coef1_fe <- feols(incidents~area:seas:city_cat+rain_t | xy+country^year+yearmo, datasub_dt[event=="battles"],vcov=~xy)
coef2_fe <- feols(incidents~area:seas:city_cat+rain_t | xy+country^year+yearmo, datasub_dt[event=="violence"],vcov=~xy)
coef3_fe <- feols(incidents~area:seas:city_cat+rain_t | xy+country^year+yearmo, datasub_dt[event=="riots" ],vcov=~xy)
coef4_fe <- feols(incidents~area:seas:city_cat+rain_t | xy+country^year+yearmo, datasub_dt[event=="protests"],vcov=~xy)
