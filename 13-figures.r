...
ggsave("Figures/map_conflict.png",gg_maplegend,width=6.5,height=5.25,dpi="retina",device="png")
...
gg_comb <- plot_grid(gg1,gg2,ncol=1,align="v",axis="tb",rel_heights = c(4,1))

gg_comb

ggsave("Figures/series_conflict.png",gg_comb,width=6.5,height=6.5*9/16,dpi="retina",device="png")
...
gg_map <- ggplot(data = southeastasia) +
  ...
  geom_point(data=cal_dt,aes(x=longitude,y=latitude,alpha=prop_i,size=area_spam),shape=21,color="dimgray",fill=NA,size=3.5,na.rm=T)+
...
ggsave("Figures/map_harvest.png",gg_maplegend,width=6.5,height=5.25,dpi="retina",device="png")
...
gg_irri <- ggplot(sub_dt,aes(x=irri))+
  ...
  labs(x="Proportion of irrigated land",y="Count (cells)")+
  theme_paper()

ggsave("Figures/appendix_irrigation.png",gg_irri,width=6.5,height=6.5*9/16,dpi="retina")
# ggsave("Figures/appendix_irrigation.eps",gg_irri,width=6.5,height=4.0,dpi="retina",device="eps")



sub2_dt <- datacomb_dt[,.(rain=gs_rain)]

gg_rain <- ggplot(sub2_dt,aes(x=rain/100))+
  geom_histogram(binwidth=.2,fill="darkgray",color="white",boundary=0)+
  labs(x="Monthly growing season rainfall (100mm)",y="Count (cell-years)")+
  theme_paper()

ggsave("Figures/appendix_rainfall.png",gg_rain,width=6.5,height=6.5*9/16,dpi="retina")
# ggsave("Figures/appendix_irrigation.eps",gg_irri,width=6.5,height=4.0,dpi="retina",device="eps")
