...
impact1i <- function(x,n){
  r <- feols(incidents~area_spam:seas+rain_t | sw(xy+country^year+yearmo,xy+country^year+mo,xy+yearmo,xy+year+mo), data=x,vcov=~xy)
  ...
  for(i in 1:n){
    ...
    h_coef <- round(r[[i]]$coeftable["area_spam:seas","Estimate"]*s,1)
    
    h_se <- round(r[[i]]$coeftable["area_spam:seas","Std. Error"]*s,1)
    
    h_stars <- pstars(r[[i]]$coeftable["area_spam:seas","Pr(>|t|)"])
...
impact1c150 <- function(x,n){
  r <- feols(incidents~area_spam:seas+rain_t | sw(xy+country^year+yearmo,xy+country^year+mo,xy+yearmo,xy+year+mo), data=x,vcov=vcov_conley(lat="latitude",lon="longitude",cutoff=150,distance="spherical"))
...
impact1c300 <- function(x,n){
  r <- feols(incidents~area_spam:seas+rain_t | sw(xy+country^year+yearmo,xy+country^year+mo,xy+yearmo,xy+year+mo), data=x,vcov=vcov_conley(lat="latitude",lon="longitude",cutoff=300,distance="spherical"))
...
impact1c <- function(x,n){
  r <- feols(incidents~area_spam:seas+rain_t | sw(xy+country^year+yearmo,xy+country^year+mo,xy+yearmo,xy+year+mo), data=x,vcov=~country)
...
gg_est <- ggplot(dt,aes(x=group_id,y=impact,group=group_id))+
  ...
  labs(subtitle="Harvest-time change in conflict relative to the baseline (%)",x="",y="")+
...
gg_spec1 <- ggplot(spec1_dt,aes(x=group_id,y=variable)) + 
  ...
  geom_point(aes(size=value),na.rm=T,shape=15,color="dimgray")+
  scale_size_continuous(range=c(0,3))+
  labs(subtitle="Fixed effects",x="",y="")+
  theme_paper()+
...
gg_spec2 <- ggplot(spec2_dt,aes(x=group_id,y=variable)) + 
  ...
  geom_point(aes(size=value),na.rm=T,shape=15,color="dimgray")+
  scale_size_continuous(range=c(0,3))+
  labs(subtitle="Level of clustering",x="",y="")+
  theme_paper()+
  theme(axis.title.x=element_blank(),axis.line.x = element_blank(),axis.text.x = element_blank(),axis.text.y = element_text(hjust=0))
